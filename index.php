<!doctype HTML>
<html lang="fr">
      <head> 
	      <title> VIRGINIE DUHAMEL  </title> <!--le titre de la page -->
		<link rel="stylesheet" type="text/css" href="css/style_div.css"> <!-- on imparte le css -->
		<link rel="stylesheet" type="text/css" href="css/style_image.css"> <!-- on imparte le css -->
		<link rel="stylesheet" type="text/css" href="css/style_police.css"> <!-- on imparte le css -->
		<link rel="stylesheet" type="text/css" href="css/style_bouton.css"> <!-- on imparte le css -->
		<link rel="stylesheet" type="text/css" href="css/style_footer.css"> <!-- on imparte le css -->
		<link rel="stylesheet" type="text/css" href="css/style_bg.css"> <!-- on imparte le css -->
		<script src="js/redirection.js"></script> <!-- on import le js -->
	      <link rel="icon" type="image/png" href="images/collage/image1.jpg" /> <!-- logo à coté du titre-->
	</head>
	        
	<body>
		<div class="align">
			<div onclick="redirect(0,1);" class="btn from-left">Virginie</div>
			<div onclick="redirect(1,1);" class="btn from-left">Collage</div> 
			<div onclick="redirect(2,1);" class="btn from-left">Peinture</div>
			<div onclick="redirect(3,1);" class="btn from-left">Contact</div> 	
		</div>			
		<div id="contenu_pres">
			<h2>
			Virginie Duhamel a étudié à Paris 8 où elle a obtenu sa maitrise d’Histoire de l’Art.<br>
			Elle a également suivi les ateliers des Beaux Arts et participé aux cours d’arts modernes de l’école et galerie Edouard Manet à Gennevilliers, suite à quoi elle a obtenu un atelier dans le vieux Gennevilliers.<br>
			Elle a participé à des concours nationaux comme celui des Arts et Métiers catégorie « Peinture » et obtenu en 2012 le premier prix, ainsi qu’en 2005, le second prix du Salon des jeunes artistes contemporains du 92.<br>
			Son travail consiste à créer des contrastes visuels, entre portraits dessinés à la mine de plomb et motifs colorés sur papier. Afin de faire ressortir son travail sur le visage, elle procède « a contrario », c’est-à-dire en attirant d’abord notre œil sur des motifs en couleurs, évoquant parfois le dessin d’enfant, pour finalement l’attirer vers des visages à priori plus en retraits, mais tout aussi présents que ses motifs, et souvent emprunts d’une forme de mélancolie, ou de réflexion tournée vers l’intériorité.<br>  
			On peut également y voir un travail sur le masque façon « commedia dell’arte », comme plusieurs personnes l’ont souligné.<br>
			Toutes ses dernières compositions sont donc des collages sur carton ou toile, l’idée étant de proposer un travail avant tout sur le motif, la couleur et le portrait, en utilisant des procédés visuels contradictoires.<br>
			Auparavant, elle peignait à l’huile sur bois, mais finalement cette technique a cédé sa place à des moyens créatifs plus rapides et plus légers, comme les crayons et les feutres, type marqueurs, sur feuilles A4 ou A3.<br>
			</h2>	
		</div>
      </body>
      <footer>
      	<h2 class="footer">&copy; Copyright 2021 - Virginie Duhamel </h2>
      	<h2 class="footer"> <a href="mailto:virginesophie.duhamel@orange.fr"><img class="insta" src="images/mail.png"></a>
      	<a href="https://www.instagram.com/collage_peinture_dessin/" target="_blank"><img class="insta" src="images/insta.png"></a></h2>
      </footer>
      <script src="///www.lescigales.org/banner/horizontal.js?v=1" where="bottom" async defer></script>
</html>